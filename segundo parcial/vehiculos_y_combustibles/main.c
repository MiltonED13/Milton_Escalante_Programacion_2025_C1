...
#define PRECIO_COMBUSTIBLE 290 // Constante del precio del combustible
...
void guardarVehiculos(Vehiculo vehiculos[], int totalVehiculos) {
    ...
    // Guardar el precio del combustible en el archivo
    float precioCombustible = PRECIO_COMBUSTIBLE;  // Variable auxiliar
...
void leerVehiculosYMostrar() {
    ...
    for (int i = 0; i < totalVehiculos; i++) {
        ...
        float costoTotal = vehiculo.consumo * PRECIO_COMBUSTIBLE;  // Calcular el costo total
...
int main() {
    ...
    guardarVehiculos(vehiculos, totalVehiculos);

    // Leer los datos del archivo y mostrar la informacion
    leerVehiculosYMostrar();
