...
void main(void)
{
    ...
    if (isalpha(p1))
    {
        if (islower(p1))
        {
            /* La funcion islower regresa 1 si p1 es una letra minuscula y 0 en caso contrario.
               La funcion isupper, por otra parte, regresa 1 si p1 es una letra mayuscula y 0 en caso contrario. */
    ...
    if(isalpha(p1))
    {
        if(isupper(p1))
        {
            ...
            /* la funcion tolower convierte de mayuscula a minuscula. Si la letra
            es minuscula no la modifica. La funcion toupper, por otra parte, convierte
            de minuscula a mayuscula. Si la letra es mayusucla no la modifica. */
        }
        else
        {
            printf("%c es una letra minuscula \n", p1);
