...
//Cada prototipo de funcion corresponde a uno de los incisos.
...
void main(void)
{
    ...
    Funcion1(UNI, F, C, P);
    Funcion2(UNI, F, C, P);
    Funcion3(UNI, 6, C, P); //Se coloca el 6 como parametro ya que es la clave de la Ingenieria en Computacion.
}
...

{
    ...
    for (K = 0; K < PR; K++)
    {
        for (I = 0; I < FI; I++)
        {
            for (J = 0; J < CO; J++)
            {
                printf("Ano: %d\tCarreras: %d\tSemestre: %d ", K+1, I+1, J+1);
                scanf("%d", &A[I][J][K]);
            }
        }
    }
...
/* Esta funcion se utiliza para determinar el año en el que ingresó el mayor
...

{
    ...
    for (K = 0; K < PR; K++)//Por cada año se suma el ingreso de los semestres de las ocho carreras.
    {
        SUM = 0;
...
numero de alumnos el último año. Observa que no se utiliza un ciclo para los
planos de la profundidad, ya que es un dato (PB). */
