...
void main (void)
{
    ...
    /* Observa que las variables *q3, *a4 y *a5 se declaran como apuntadores
    de tipo estructura alumno. a6 es una variable de tipo estructura alumno. */

    a3 = &a0;
    /* En este caso el apuntador de tipo estructura alumno a3 se le asigna
    la direccion de la variable de tipo estructura alumno, a0. */
    ...
    Lectura(&a6);
    /* En este caso se pasa la variable de tipo estructura alumno a6, a la funcion
    Lectura. Observa que en este caso debemos utilizar el operador de
    direccion para preceder a la variable. */

    printf("\nDatos del alumno 3\n");
    /* Observa la forma de escribir los campos de los apuntadores de
    tipo estructura. */
    ...
    printf("\nDatos del alumno 4\n");
    printf("%d\t%s\t%.2f\t%s", a4->matricula, a4->nombre, a4->carrera,
           a4->promedio, a4->direccion);

    printf("\nDatos del alumno 5\n");
    printf("%d\t%s\t%.2f\t%s", a5->matricula, a5->nombre, a5->carrera,
           a5->promedio, a5->direccion);

    printf("\nDatos del alumno 6\n");
    //Observa la forma de escribir los campos de la variable tipo estructura.
    printf("%d\t%s\t%.2f\t%s", a6.matricula, a6.nombre, a6.carrera,
           a6.promedio, a6.direccion);
}

void Lectura(struct alumno*a)
{
    /* Esta funcion permite leer los campos de un apuntador de tipo
    estructura alumno. */
